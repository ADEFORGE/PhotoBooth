...
def save_as_jpeg(uploaded_file, dest_dir: Path) -> Path:
    """
    Open the incoming file using OpenCV,
    convert to BGR if necessary, and save as a .jpg with a random-seeded filename.
    Returns the Path to the saved JPEG.
    """
    dest_dir.mkdir(parents=True, exist_ok=True)

    # Generate a random filename seed
    seed_str = ''.join(random.choices(string.ascii_lowercase + string.digits, k=8))
    jpeg_path = dest_dir / f"converted_{seed_str}.jpg"

    # Read image file into numpy array
    # If uploaded_file is a path-like, read directly, else read bytes and decode
    try:
        img = cv2.imread(str(uploaded_file), cv2.IMREAD_UNCHANGED)
        if img is None:
        ...
        log(f"Error reading image with OpenCV: {e}", level="error")
        raise

    # Convert BGRA or grayscale to BGR for JPEG
    ...
    try:
        cv2.imwrite(str(jpeg_path), img, [int(cv2.IMWRITE_JPEG_QUALITY), 85])
    except Exception as e:
        log(f"Error writing JPEG: {e}", level="error")
        raise
...
<body>
  ...
  <img src="/{self.image}?timestamp=12345" alt="Shared photo">
