...
class HotspotShareImage:
    ...
    def update_splash_html(self) -> None:
        ...
        Exit: None
        Replaces the splash HTML with a minimal page that shows only the image.
        """
        log("[update_splash_html] Enter (minimal version)", level="info")
    ...
    """
        ...
        try:
            (self.image_dst_dir / 'splash.html').write_text(minimal_html)
            log("Minimal splash.html written", level="info")
        except Exception as e:
            log(f"Error writing minimal splash.html: {e}", level="error")
            raise
        log("[update_splash_html] Exit (minimal version)", level="info")
